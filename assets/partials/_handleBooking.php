...
if (!$booking_exists) {
    // Route is unique, proceed
    $sql = "INSERT INTO `bookings` (`customer_id`, `route_id`, `customer_route`,  `booked_seat`,`travel_purpose`, `booking_created`) VALUES ('$customer_id', '$route_id','$customer_route',  '$customer_seat','$travel_purpose', current_timestamp());";
    ...
    if (!$result) {
        ...
        if ($autoInc_id) {
            ...
            if (!$queryResult) {
                echo "Error updating booking_id: " . mysqli_error($conn); // Display the error message
            }
        }
