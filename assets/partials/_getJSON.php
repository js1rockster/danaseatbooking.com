<?php
	...
	$driverJson = '';
    // Driver JSON
    $driverSql = "Select * from drivers";
    $resultdriverSql = mysqli_query($conn, $driverSql);
    $arr = array();
    if($resultdriverSql) { // Check if the query was successful
        if(mysqli_num_rows($resultdriverSql)) {
            while($row = mysqli_fetch_assoc($resultdriverSql))
                $arr[] = $row;
        }
        $driverJson = json_encode($arr);
    }
?>
